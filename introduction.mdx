...
| Feature | Description |
|---------|-------------|
| Multi-Format Support | Supports HEIC (iOS), JPEG, PNG, and WebP formats. Automatically converts and optimizes images for AI processing. |
| AI-Powered OCR | Uses Groq's Llama 4 Vision model to extract structured product data from screenshots. |
| Smart Cropping | Gemini 2.0 Flash detects and crops the main product from screenshots, removing UI elements. |
| Search Enrichment | Automatically searches for product URLs and scrapes additional images and metadata using Gemini's function calling. |
| PostgreSQL Storage | All products are stored in a PostgreSQL database with full relational support for lists and collections. |
